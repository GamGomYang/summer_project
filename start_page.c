...
void run_battleship_game() {
    ...
    int result = system("./battle_ship/battleship_client");

    if (result != 0) {
        printf("배틀쉽 게임 실행에 실패했습니다. (오류 코드: %d)\n", result);
    }

    // 게임 종료 후 메인 메뉴로 돌아가기
    printf("\n게임이 종료되었습니다. 아무 키나 누르면 메인 메뉴로 돌아갑니다...\n");
    getchar();
}
...
void run_typing_game() {
    ...
    int result = system("./typing_game/client");

    if (result != 0) {
        printf("타이핑 게임 실행에 실패했습니다. (오류 코드: %d)\n", result);
    }
...
void run_coda_game() {
    ...
    int result = system("./coda_module/client");

    if (result != 0) {
        printf("coda 게임 실행에 실패했습니다. (오류 코드: %d)\n", result);
    }
